...
void FoosballTournament::registerTeam(Team team)
{
	if (!team.isValid())
	{
...
FoosballTournament::FoosballTournament(size_t capacity)
	: f_capacity(capacity), f_size(0)
{
	f_teams = new Team[capacity];
}

FoosballTournament::FoosballTournament(const FoosballTournament& other)
	: FoosballTournament()
{
	if (this != &other)
	{
...
FoosballTournament& FoosballTournament::operator=(const FoosballTournament& other)
{
	if (this != &other)
	{
		del();
...
FoosballTournament::~FoosballTournament()
{
	del();
}

void FoosballTournament::del()
{
	delete[] f_teams;
	f_teams = nullptr;
}

void FoosballTournament::copy(const FoosballTournament& other)
{
...
void FoosballTournament::resize(size_t newCapacity)
{
...
bool FoosballTournament::teamIsValidForTournament(const Team& team) const
{
	int ceoCount = 0;
	int ctoCount = 0;
...
bool FoosballTournament::teamCountIsPowerOfTwo() const
{
	// Bit operations way.
...
void FoosballTournament::removeTeamsUntilTeamCountIsPowerOfTwo()
{
...
int FoosballTournament::findClosestPowerOfTwo(int number) const
{
	int powerOfTwo = 1;
	int twoToThePower = 2;
...
void FoosballTournament::removeLastTeam()
{
	if (f_size == 0)
	{
		return;
...
void FoosballTournament::randomlyShuffleTeams()
{
	srand(time(NULL));
...
const Team& FoosballTournament::commenceDuel(const Team& first, const Team& second) const
{
...
const Team& FoosballTournament::commenceRound(const Team& first, const Team& second) const
{
...
int FoosballTournament::rollDie(int repetitions, int sideCount) const
{
	srand(time(NULL));
...
void FoosballTournament::commenceSingleQualification(Team** teams, size_t& size) const
{
...
void FoosballTournament::removeElement(Team** teams, size_t& size, int index) const
{
